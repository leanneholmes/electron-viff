...
const CustomBar = () => {
  ...
  const screenData = scheduleContext.gridScreenTimes
    .find((date) => date.id == targetID.dateID)
  ...
  useEffect(() => {
    setScreenInfo({
      ...
      movieLink: screenData.movieLink,
    });
  }, [screenData]);
  ...
  const changeScreen = () => {
    ...
    if (readyToChange) {
      setScreenInfoValue(
        "filmTitleText",
        Number.parseFloat(screenInfo.filmTitleText)
      );
      setScreenInfoValue(
        "filmDetailsText",
        Number.parseFloat(screenInfo.filmDetailsText)
      );
      scheduleContext.dispatch({
        type: CHANGE_SCREEN,
        screen: screenInfo,
  ...
  return (
    <div className={targetID.display ? "rightsidebar" : "hide"}>
      <div style={{ cursor: "pointer" }} onClick={() => closeMenu()}>
        <CloseIcon></CloseIcon>
      </div>
      ...
      >
        ...
        <FormControl variant="standard">
          ...
          <Input
            id="component-simple"
            value={screenInfo.filmTitleText}
            onChange={(e) => {
              setTitleError(false);
        ...
        <FormControl variant="standard">
          <InputLabel htmlFor="component-simple">
            FilmDetail Font Size
          </InputLabel>
          <Input
            id="component-simple"
            value={screenInfo.filmDetailsText}
            onChange={(e) => {
