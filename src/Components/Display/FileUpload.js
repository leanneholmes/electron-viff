  ...
  const onDrop = useCallback((acceptedFiles) => {
    ...
    acceptedFiles.forEach((file) => {
      if (!uploadedFiles.get(file.path)) {
        ...
        document.getElementById("file-list-header").innerHTML =
          "Files Uploaded";
      ...
      reader.onload = () => {
        ...
        } else if (fileColumn.length == 9 || fileColumn.length == 10) {
  ...
  const files = [...uploadedFiles.values()].map((file) => (
    <li key={file.path}>
      ...
      <span className="checkmark">
        <MaterialIcon icon={icon} color="#2dce89" size={16} />
      </span>
    </li>
  ));
  ...
  return (
    <div>
      ...
      <div className="file-list">
        <h4>
          <span id="file-list-header">Uploaded Files</span>
        </h4>
